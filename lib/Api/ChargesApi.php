...
class ChargesApi
{
     ...
     * @return \Ageras\Api\Model\ChargeResult
     */
    public function chargesIndex($criteria = [])
    {
        list($response) = $this->chargesIndexWithHttpInfo($criteria);
        return $response;
    }
     ...
     * ]
     * @throws \Ageras\Api\ApiException on non-2xx response
     * @return array of \Ageras\Api\Model\ChargeResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function chargesIndexWithHttpInfo($criteria = [])
    {
        // parse inputs
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Ageras\Api\Model\ChargeResult',
                '/charges'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Ageras\Api\Model\ChargeResult', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Ageras\Api\Model\ChargeResult', $e->getResponseHeaders());
