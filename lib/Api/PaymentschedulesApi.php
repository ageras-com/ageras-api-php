...
class PaymentschedulesApi
{
     ...
     * @return \Ageras\Api\Model\PartnerPaymentScheduleResult
     */
    public function paymentschedulesIndex($criteria = [])
    {
        list($response) = $this->paymentschedulesIndexWithHttpInfo($criteria);
        return $response;
    }
     ...
     * ]
     * @throws \Ageras\Api\ApiException on non-2xx response
     * @return array of \Ageras\Api\Model\PartnerPaymentScheduleResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function paymentschedulesIndexWithHttpInfo($criteria = [])
    {
        // parse inputs
        $resourcePath = "/paymentschedules";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Ageras\Api\Model\PartnerPaymentScheduleResult',
                '/paymentschedules'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Ageras\Api\Model\PartnerPaymentScheduleResult', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Ageras\Api\Model\PartnerPaymentScheduleResult', $e->getResponseHeaders());
