...
/**
 * PaymentsolutionsApi Class Doc Comment
 *
 * @category Class
 * @package  Ageras\Api
 * @author   Swagger Codegen team
...
class PaymentsolutionsApi
{
     ...
     * @return \Ageras\Api\Model\PaymentSolutionResult
     */
    public function paymentsolutionsIndex($criteria = [])
    {
        list($response) = $this->paymentsolutionsIndexWithHttpInfo($criteria);
        return $response;
    }
     ...
     * ]
     * @throws \Ageras\Api\ApiException on non-2xx response
     * @return array of \Ageras\Api\Model\PaymentSolutionResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function paymentsolutionsIndexWithHttpInfo($criteria = [])
    {
        // parse inputs
        $resourcePath = "/paymentsolutions";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Ageras\Api\Model\PaymentSolutionResult',
                '/paymentsolutions'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Ageras\Api\Model\PaymentSolutionResult', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Ageras\Api\Model\PaymentSolutionResult', $e->getResponseHeaders());
