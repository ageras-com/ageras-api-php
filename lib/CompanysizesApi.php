...
/**
 * CompanysizesApi Class Doc Comment
 *
 * @category Class
 * @package  Ageras\Api
 * @author   Swagger Codegen team
...
class CompanysizesApi
{
     ...
     * @return \Ageras\Api\CompanySizeResult
     */
    public function companysizesIndex($criteria = [])
    {
        list($response) = $this->companysizesIndexWithHttpInfo($criteria);
        return $response;
    }
     ...
     * ]
     * @throws \Ageras\Api\ApiException on non-2xx response
     * @return array of \Ageras\Api\CompanySizeResult, HTTP status code, HTTP response headers (array of strings)
     */
    public function companysizesIndexWithHttpInfo($criteria = [])
    {
        // parse inputs
        $resourcePath = "/companysizes";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Ageras\Api\CompanySizeResult',
                '/companysizes'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Ageras\Api\CompanySizeResult', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Ageras\Api\CompanySizeResult', $e->getResponseHeaders());
